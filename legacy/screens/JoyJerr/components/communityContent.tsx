...
// import DocumentPicker from '@react-native-documents/picker'; // Temporarily disabled due to compatibility issues
// @ts-ignore
...
const CommunityContent: React.FC = () => {
  ...
  // File attachment functions - Temporarily disabled due to DocumentPicker compatibility issues
  const pickFile = async () => {
    Alert.alert(
      'Fonctionnalité temporairement indisponible',
      "L'ajout de fichiers est temporairement désactivé en raison de problèmes de compatibilité. Cette fonctionnalité sera restaurée prochainement.",
    );
    // try {
  ...
  const getCurrentUserLocation = async () => {
    ...
    try {
      ...
      const locationName = `${(location as any).coords.latitude.toFixed(6)}, ${(
        location as any
  ...
  const removeHelpOption = (optionToRemove: string) => {
    setHelpOptions(prev => prev.filter(option => option !== optionToRemove));
    setSelectedHelpOptions(prev =>
  ...
  const updateHelpOption = (oldOption: string, newOption: string) => {
    if (newOption.trim() && newOption.trim() !== oldOption) {
      setHelpOptions(prev =>
        prev.map(option => (option === oldOption ? newOption.trim() : option)),
      );
      setSelectedHelpOptions(prev =>
        prev.map(option => (option === oldOption ? newOption.trim() : option)),
      );
            ...
            <View style={styles.filterContent}>
              ...
              <View style={styles.filterOptionsContainer}>
                ...
                >
                  <Ionicons
                    name={
                      sortByOptions.find(option => option.id === selectedSortBy)
                        ?.icon || 'chatbubble'
                    }
                    size={16}
                    color="#007bff"
